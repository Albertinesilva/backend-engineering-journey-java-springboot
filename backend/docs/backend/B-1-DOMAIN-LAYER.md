...
| 8 | `Product.price` é `Double` (coluna `float(53)`) — ponto flutuante para valor monetário, sem validação na entidade. **Refinado na B-4:** o DTO de criação/atualização tampouco exige o preço (`@Positive` aceita `null`), então é possível criar produto sem preço. | FATO |
...
| 10 | `User` usado como `UserDetails`: `isAccountNonExpired/Locked/CredentialsNonExpired` sempre `true`; não há modelo de bloqueio ou expiração. Além disso, `UserService.loadUserByUsername` monta um `User` **parcial** (id, e-mail, senha, `active`, roles; nome/sobrenome nulos) — ponto para a fase de services/segurança. **Refinado na B-3:** o `User` parcial é consumido somente pelo `CustomPasswordAuthenticationProvider` (senha, status, authorities e id); não há leitura de nome/sobrenome sobre ele, e ele nunca é persistido (B-3 §5.3). | FATO |
